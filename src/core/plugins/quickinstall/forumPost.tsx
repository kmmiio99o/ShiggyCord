...
import {
  HTTP_REGEX_MULTI,
  VD_DISCORD_SERVER_ID,
  VD_PLUGINS_CHANNEL_ID,
  VD_PROXY_PREFIX,
  VD_THEMES_CHANNEL_ID,
...
const { useFirstForumPostMessage } = lazyDestructure(() =>
  findByProps("useFirstForumPostMessage"),
);
...
function useExtractThreadContent(
  ...
  const { firstMessage } = actionSheet
    ? useFirstForumPostMessage(thread)
    : { firstMessage: _firstMessage };

  const urls = firstMessage?.content
    ?.match(HTTP_REGEX_MULTI)
    ?.filter(postMap[postType].urlsFilter);
...
const installButtonPatch = () =>
  after(
    "MostCommonForumPostReaction",
    forumReactions,
    ([{ thread, firstMessage }], res) => {
      const [shouldReturn, _, installed, loading, installOrRemove] =
        useInstaller(thread, firstMessage, true);
      if (shouldReturn) return;
        ...
        <>
          ...
          <ErrorBoundary>
            <Button
              ...
              onPress={installOrRemove}
              icon={findAssetId(
                installed ? "ic_message_delete" : "DownloadIcon",
              )}
              style={{ marginLeft: 8 }}
            />
          </ErrorBoundary>
