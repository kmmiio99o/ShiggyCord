...
export function initFetchI18nStrings() {
  const cb = ({ locale }: { locale: string }) => {
    ...
    if (!_loadedLocale.has(resolvedLocale)) {
      ...
      fetch(
        `https://raw.githubusercontent.com/C0C0B01/i18n/main/resources/${resolvedLocale}/shiggycord.json`,
      )
        .then((r) => r.json())
        ...
        .catch((e) =>
          console.error(
            `An error occured while fetching strings for ${resolvedLocale}: ${e}`,
          ),
        );
    } else {
      _currentLocale = resolvedLocale;
