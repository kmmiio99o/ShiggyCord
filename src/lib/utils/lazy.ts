...
export function proxyLazy<T, I extends ExemptedEntries>(
  ...
  // Use the proxyFactory in the context holder so the lazy handler calls the cached
  // resolver instead of the raw factory. This avoids attempting to Reflect.* on
  // an unresolved/undefined value when the original factory hasn't produced the object yet.
